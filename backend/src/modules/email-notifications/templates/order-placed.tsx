    ...
    <Base preview={preview}>
      <Section>
        ...
        <Text style={{ margin: '0 0 20px' }}>
          Total: ${Number(order.summary.raw_current_order_total.value).toFixed(2)} {order.currency_code.toUpperCase()}
        </Text>
        ...
        <div style={{
          ...
          {order.items.map((item) => (
            <div key={item.id} style={{
              ...
              <Text>${Number(item.unit_price).toFixed(2)} {order.currency_code.toUpperCase()}</Text>
            </div>
          ))}
        </div>
      </Section>
